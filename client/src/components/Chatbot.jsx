  ...
  return (
    <>
      <div className="fixed right-4 bottom-4 z-50">
        <button
          onClick={toggleModal}
          className="text-3xl p-2 rounded-full bg-blue-500 text-white hover:bg-blue-600 focus:outline-none"
        >
          <FaRocketchat />
        </button>
      </div>
      ...
      {/* {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-40">
          <div className="flex h-screen justify-center items-start pt-16">
            <section
              className="relative bg-gradient-to-r from-slate-900 to-slate-700 rounded-md shadow-lg w-full h-full"
              style={{ maxHeight: "90vh" }}
            >
              <button
                onClick={toggleModal}
                className="absolute top-0 right-0 text-xl p-4 bg-transparent text-white hover:text-gray-300"
              >
                <FaTimes />
              </button>
              <div
